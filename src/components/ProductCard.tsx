  ...
  return (
    <>
      ...
      {isOpen && mounted && createPortal(
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md" onClick={() => setIsOpen(false)}>
          <div className="bg-content1 border border-white/10 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto flex flex-col sm:flex-row" onClick={e => e.stopPropagation()}>
              ...
              {allImages.length > 0 ? (
                <>
                  <div className="relative w-full aspect-square bg-white/5 rounded-xl overflow-hidden mb-4 shadow-xl border border-white/5">
                    <Image src={allImages[currentImageIndex]} alt={item.name} fill className="object-cover" />
                  </div>
                  {allImages.length > 1 && (
                    <div className="flex gap-2 overflow-x-auto py-2 w-full">
                      {allImages.map((img, idx) => (
                        ...
                        >
                          <Image src={img} alt={`${item.name} thumb ${idx}`} fill className="object-cover" />
                        </div>
                      ))}
                    </div>
                  )}
